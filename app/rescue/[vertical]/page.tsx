...
export async function generateMetadata({ params }: { params: Promise<{ vertical: string }> }) {
  ...
  return { title: v ? `${v.title} — AIKAGAN` : "AIKAGAN" };
}

export default async function RescuePage({ params }: { params: Promise<{ vertical: string }> }) {
  const { vertical } = await params;
  const v = getVertical(vertical);
  if (!v) return <main className="mx-auto max-w-3xl px-6 py-24">Vertical not found.</main>;
    ...
    <main className="bg-ink">
      <section className="border-b border-white/5" style={{ background: `radial-gradient(circle at 75% 15%, ${v.accent}1f, transparent 40%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-24">
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-neutral-300">{v.pain}</p>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[
              ["Free", v.freeTool, "A useful diagnosis you keep."],
              ["Prove", v.paidOffer, "A bounded outcome with acceptance."],
              ["Evidence", v.engine, "The engine that maps the cure."],
            ].map(([k, t, b]) => (
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
          <div>
            <h2 className="text-3xl font-black">The pain-killer proposition</h2>
            <p className="mt-5 leading-8 text-neutral-400">{v.proof}</p>
            <ul className="mt-6 space-y-3">
              {[
                "Free diagnosis before any ask",
