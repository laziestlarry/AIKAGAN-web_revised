...
export default function RadarPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-20">
      <p className="chip text-emerald-300">Demand radar</p>
      <h1 className="mt-5 text-4xl font-black sm:text-5xl">Where the value is stranded</h1>
      <p className="mt-5 max-w-3xl leading-8 text-neutral-300">
        We hunt observable pain, not manufactured fear. Each row shows a buyer who already owns something valuable but
        underperforms it. Ranked by pain × equity × commercial niche × repeatability.
      </p>
      ...
      <div className="mt-12 space-y-4">
        {ranked.map((v) => {
          ...
          return (
            <Link key={v.id} href={`/rescue/${v.id}`} className="group grid gap-4 rounded-3xl border border-white/10 bg-white/[0.02] p-6 transition hover:border-white/20 md:grid-cols-[64px_1fr_180px_120px] md:items-center">
